...
if (on >= 1 && on <= 4)
{
    ...
    draw_sprite_ext(spr_onionblue, 0, x, y + 75, ((0.1 + onion_blue_alpha) + (sin((siner / 4)) * 0.05)), (onion_blue_alpha + (sin((siner / 4)) * 0.05)), 0, c_white, onion_blue_alpha)
}
if (on == 2 || on == 3 || on == 4)
{
    if (global.flag[20] == 0)
        onionsprite = spr_nothing
    if (onionsetsprite = 1)
        onionsprite = spr_nothing
    if (onionsetsprite = 2)
        onionsprite = spr_nothing
